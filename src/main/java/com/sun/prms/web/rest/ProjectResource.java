...
public class ProjectResource {
    ...
    @GetMapping("/sunhrm-projects")
    @Timed
    public List<HrmOhrmProject> getProjectsFromHrm() {
    	return projectService.getProjectsFromHrm();
    }
    
    @GetMapping("/sunhrm-clients")
    @Timed
    public List<HrmCustomer> getClientsFromHrm() {
    	return projectService.getClientsFromHrm();
    }
    
    @GetMapping("/projects-of-clients/{clientId}")
    @Timed
    public List<HrmOhrmProject> getProjectsOfclientsFromHrm(@PathVariable Integer clientId) {
    	return projectService.getProjectsOfClientsFromHrm(clientId);
    }
    
    @PostMapping("project-search")
    @Timed
    public ResponseEntity<List<ProjectModelForSearch>> getProjectsBySearch(
    	 @RequestBody ProjectSearch projectSearch
    ) {
    	
    	// return projectService.searchByQuery(projectName, clientName, projectStatus, projectType,percentageComplete);
    	List<ProjectModelForSearch> result = projectService.searchByQuery(
			projectSearch.getProjectName(), 
			projectSearch.getClientName(),
			projectSearch.getProjectStatus(),
			projectSearch.getProjectType(),
			projectSearch.getPercentageComplete()
